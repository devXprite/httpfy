...
const { Color } = require("./httpfyConfig");
...
const throwError = (error) => {
    console.log(Color ? chalk.red(error) : error);
    process.exit(0);
};
...
const multiBar = new cliProgress.MultiBar(
    {
        format: Color
            ? `${chalk.cyanBright("{percentage}%")} |${chalk.green("{bar}")}| ${chalk.hex("#FFA500")("{value}/{total} Requests")} | ${chalk.gray("ETA: {eta_formatted}")}`
            : "{percentage}% |{bar}| {value}/{total} Requests | ETA: {eta_formatted}",
        barCompleteChar: "\u2588",
        barIncompleteChar: "\u2591",
        stopOnComplete: true,
        hideCursor: true,
