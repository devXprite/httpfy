 ...
 * @property {(string|boolean)} OutputFolder
 */
...
const httpfyConfig = {
    ...
    RequestTimeout: args.timeout ? args.timeout * 1000 : 10 * 1000,
    Interval: Number(args.interval) * 1000 || 0,
    UserAgent: args.userAgent || sample(userAgents),
    Cookie: args.cookie || "",
    Headers: (args.header) ? args.header.split(/:/).map((v) => v.trim()) : false,
    ...
    maxRedirect: (args.redirect !== null && args.redirect !== undefined ? args.redirect : false) ? Number(args.maxRedirect || 3) : 0,
    ...
    OutputFolder: args.outputFolder || false,
};
